...
DescriptorSetLayout::DescriptorSetLayout(
  ...
  if (vkCreateDescriptorSetLayout(device.device(), &descriptorSetLayoutInfo, nullptr,
...
DescriptorPool::~DescriptorPool() {
  vkDestroyDescriptorPool(mVuDevice.device(), descriptorPool, nullptr);
}
...
bool DescriptorPool::allocateDescriptor(const VkDescriptorSetLayout descriptorSetLayout,
  ...
  if (vkAllocateDescriptorSets(mVuDevice.device(), &allocInfo, &descriptor) != VK_SUCCESS) {
    return false;
  }
  return true;
}
...
void DescriptorPool::freeDescriptors(std::vector<VkDescriptorSet> &descriptors) const {
  vkFreeDescriptorSets(mVuDevice.device(), descriptorPool,
                       static_cast<uint32_t>(descriptors.size()), descriptors.data());
}

void DescriptorPool::resetPool() { vkResetDescriptorPool(mVuDevice.device(), descriptorPool, 0); }
...
DescriptorWriter &DescriptorWriter::writeBuffer(uint32_t binding,
  ...
  assert(mSetLayout.bindings.count(binding) == 1 && "Layout does not contain specified binding");
...
void DescriptorWriter::overwrite(VkDescriptorSet &set) {
  ...
  vkUpdateDescriptorSets(mPool.mVuDevice.device(), mWrites.size(), mWrites.data(), 0, nullptr);
}
