  ...
  public:
	UniformBuffer(Device &device, VkShaderStageFlagBits shaderStageType)
		...
		for (int i = 0; i < mBuffers.size(); i++) {
			mBuffers[i] = std::make_unique<Buffer>(
				mVuDevice, sizeof(T), 1, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
	...
	const std::vector<std::unique_ptr<Buffer>> &getBuffers() const override {
		return mBuffers;
	}

	const VkShaderStageFlagBits &getShaderStageType() const override {
		return mShaderStageType;
	}
  ...
  public:
	class Builder {
	  public:
		...
		template <typename T>
		Builder &addUniformBuffer(VkShaderStageFlagBits shaderStageType) {
			mUniformBuffers.emplace_back(
				std::make_unique<UniformBuffer<T>>(mVuDevice, shaderStageType));
			return *this;
		}

		std::unique_ptr<UniformBufferManager> build() {
			return std::make_unique<UniformBufferManager>(
				mVuDevice, std::move(mUniformBuffers));
		}
	...
	UniformBufferManager(
		...
		: mVuDevice(device),
		  mGlobalDescriptorSets(SwapChain::MAX_FRAMES_IN_FLIGHT) {

		mUniformBuffers = std::move(uniformBuffers);

		mGlobalPool = DescriptorPool::Builder(mVuDevice)
						  .setMaxSets(SwapChain::MAX_FRAMES_IN_FLIGHT *
									  mUniformBuffers.size())
						  .addPoolSize(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
									   SwapChain::MAX_FRAMES_IN_FLIGHT *
										   mUniformBuffers.size())
						  .build();

		DescriptorSetLayout::Builder descriptorSetLayout =
			DescriptorSetLayout::Builder(mVuDevice);
		size_t curr = 0;
		for (const auto &ub : mUniformBuffers) {
			descriptorSetLayout.addBinding(curr,
